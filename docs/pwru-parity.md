...
| Text output | partial | Stable pwru-shaped core columns cover SKB, CPU/process/PID, timestamp, netns, mark, interface, protocol, MTU, length, tuple and function, with explicit association/origin provenance; caller, metadata, control buffer, tunnel, map, stack and BTF evidence render as deterministic indented records; pwru's optional netns-name presentation and flag-for-flag column suppression are not yet mirrored |
