...
| Arbitrary SKB expression filter | supported | `--filter-skb-expr` covers pwru's vendored `bice.SimpleCompile` language: `=`/`==`/`!=`/ordered comparisons, decimal/hex/octal/binary constants, enum symbols, `.`/`->` member paths, bitfields and `__be*` fields, all target-BTF resolved before immutable in-kernel execution. It additionally accepts parenthesized `&&`/`||`, normalized to at most four verifier-bounded comparisons; unit coverage and the isolated live gate exercise the bice-specific scalar semantics |
| Arbitrary XDP expression filter | supported | `--filter-xdp-expr` provides the same bice-compatible target-BTF language and bounded boolean extension over `xdp_buff`, with an independent immutable access plan; unit coverage and the dynamic-only XDP live gate exercise pointer chains, equality aliases, non-decimal constants and boolean groups |
...
| Track logically freed SKB by stack ID | supported | `--filter-track-skb-by-stackid` explicitly enables bidirectional bounded stack-anchor maps; requested non-SKB functions also enable it automatically. Identity is preserved through teardown calls and both directions are deleted at `kfree_skbmem`; the live gate requires ordered, read-clean `consume_skb` → `dst_release` → `kmem_cache_free` evidence on one SKB |
...
| Custom SKB/XDP metadata expressions | supported | Each context independently supports at most four strict target-BTF-validated scalar paths with four access steps, including inline structs/unions, pointers and bounded bitfields, with typed values and per-field failures; `--output-skb-metadata` reads `sk_buff` and `--output-xdp-metadata` reads `xdp_buff`; isolated live gates cover both |
...
| Rotating/compressed output files | supported | Maintained `file-rotate`/`flate2` sink with exact byte/backup bounds, optional age eviction (`--output-max-age-days` / pwru-compatible `--output-file-max-age`), optional gzip and transparent gzip replay/explain; skbx rotates only after a complete footer and starts the next file with a matching header, so every retained segment replays independently; unit and live gates force rotation |
