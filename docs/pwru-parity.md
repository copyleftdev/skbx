...
| Arbitrary SKB expression filter | partial | `--filter-skb-expr` supports up to four strict `&&`-joined comparisons (`==`, `!=`, `<`, `<=`, `>`, `>=`) over scalar `skb->…` paths; paths and literal widths are target-BTF checked at startup, immutable access plans execute in kernel, and read failures are counted; unrestricted C syntax, OR and parentheses remain intentionally unsupported |
| Arbitrary XDP expression filter | partial | `--filter-xdp-expr` provides the same bounded typed comparison language over target-BTF-validated `xdp->…` paths and executes from a separate immutable XDP access plan; dynamic-only live gate covers a two-condition pointer chain; unrestricted C syntax, OR and parentheses remain intentionally unsupported |
...
| Trace TC programs | partial | `--filter-trace-tc` enumerates currently loaded BTF-enabled `SCHED_CLS` programs through libbpf-rs, discovers each entry from program BTF, and loads one fentry tracer per target while reusing the base ring, telemetry, stack and lineage maps; events and capture headers carry exact program ID/name/entry/kind and replay routes use that identity; isolated live gate covers metadata and zero-loss evidence; TC BTF structure dumps are not yet supported |
| Trace XDP programs | partial | `--filter-trace-xdp` dynamically attaches fentry to every currently loaded BTF-enabled XDP program and emits exact ID/name/entry/kind/phase plus L2-filtered packet, tuple, interface, netns and MTU evidence; dynamic-only isolated live gate passes with zero observation loss; fexit/action evidence and universal lineage across drivers that copy the XDP frame into an unrelated SKB head remain missing |
