...
| Track SKB across transformations | partial | Bounded LRU lineage map assigns capture-local monotonic IDs, propagates through clone/copy and both current `skb_pp_cow_data` and older veth COW hooks, deletes at `kfree_skbmem`, and emits `filter`/`tracked_skb` provenance; isolated gate proves one canonical identity across three addresses and replay/explain continuity; XDP-frame-to-new-SKB correlation remains |
